...
def writefile(agg):
    if not os.path.exists('.'+os.sep+'pick_drop_neighborhoods'):
        os.mkdir('.'+os.sep+'pick_drop_neighborhoods')
    f = open('.'+os.sep+'pick_drop_neighborhoods'+os.sep+'pickup_'+filename[:6]+'.csv', 'w')
    ...
    for key, value in agg[0].items():
        ...
        f.write(line)
    f = open('.'+os.sep+'pick_drop_neighborhoods'+os.sep+'dropoff_'+filename[:6]+'.csv', 'w')
...
def writefile2(agg):
    if not os.path.exists('.'+os.sep+'pick_drop_neighborhoods'):
        os.mkdir('.'+os.sep+'pick_drop_neighborhoods')
    f = open('.'+os.sep+'pick_drop_neighborhoods'+os.sep+'pickup_neighborhoods.csv', 'w')
    line = '{},{},{},{},{}\n'.format('pickup neighborhoods','trips number','revenue','tip amount','tip percentage') 
    f.write(line)
    for key, value in agg[0].items():
        line = '{},{},{},{},{}\n'.format(key,value[0],value[1],value[2],value[3]) 
        f.write(line)
    f = open('.'+os.sep+'pick_drop_neighborhoods'+os.sep+'dropoff_neighborhoods.csv', 'w')
    line = '{},{},{},{},{}\n'.format('dropoff neighborhoods','trips number','revenue','tip amount','tip percentage') 
    f.write(line)
    for key, value in agg[1].items():
        line = '{},{},{},{},{}\n'.format(key,value[0],value[1],value[2],value[3]) 
...
def main():
    if len(sys.argv) != 3:
        print 'usage: ./pick_drop_neighborhoods.py --file or --directory'
        sys.exit(1)
